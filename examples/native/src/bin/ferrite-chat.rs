...
fn get_free_gpu_memory_gb() -> Option<f32> {
    ...
    // Parse first GPU's free memory (in MiB)
    let stdout = String::from_utf8_lossy(&output.stdout);
    let free_mib: f32 = stdout.lines().next()?.trim().parse().ok()?;

    Some(free_mib / 1024.0)
}

/// Get estimated VRAM for a model family
fn estimate_model_vram(family: ModelFamily, quantized: bool) -> f32 {
    // Manual mapping based on known model sizes
    match (family, quantized) {
        // TinyLlama 1.1B
        (ModelFamily::Llama, false) => 2.7,
        (ModelFamily::Llama, true) => 1.1,
        // Mistral 7B
        (ModelFamily::Mistral, false) => 14.5,
        (ModelFamily::Mistral, true) => 4.5,
        // Qwen2 0.5B
        (ModelFamily::Qwen, false) => 1.5,
        (ModelFamily::Qwen, true) => 0.8,
        // Gemma 2B
        (ModelFamily::Gemma, false) => 4.5,
        (ModelFamily::Gemma, true) => 1.6,
        // Phi-2 2.7B
        (ModelFamily::Phi, false) => 6.0,
        (ModelFamily::Phi, true) => 2.0,
        // GPT (fallback)
        (ModelFamily::Gpt, _) => 4.0,
    }
}
...
fn run_model(name: &str, quantized: bool) {
    ...
    let estimated_vram = estimate_model_vram(family, quantized);
    if let Some(free_vram) = get_free_gpu_memory_gb() {
        if estimated_vram > free_vram {
            println!("\n\x1b[1;33m⚠ WARNING: Potential VRAM shortage\x1b[0m");
            println!("  Model requires: ~{:.1} GB", estimated_vram);
            println!("  Available VRAM: ~{:.1} GB", free_vram);
            println!();
            ...
            io::stdin().read_line(&mut response).unwrap();
            if !response.trim().eq_ignore_ascii_case("y") {
                println!("Aborted. Consider using a quantized model (--quantized / -q).");
                return;
            }
            println!();
        }
